...
function formats_fetch()
	...
	data[url] = "fetching"
	execasync(function(a, b, c)
		formats_save(url, a, b, c)
	end, get_ytdl_cmdline())
end
...
function execasync(fn, args)
	mp.command_native_async({ name = "subprocess", args = args, capture_stdout = true, capture_stderr = true }, fn)
end

-- execute shell commands using mpv's subprocess command
function exec(args)
	return mp.command_native({ name = "subprocess", args = args, capture_stdout = true, capture_stderr = true })
end
